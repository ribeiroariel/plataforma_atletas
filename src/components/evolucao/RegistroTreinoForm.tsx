...
export function RegistroTreinoForm() {
  ...
  const [modo, setModo] = useState<"distancia" | "pace">("distancia");

  const distanciaOuPace = tipo === "corrida" || tipo === "bicicleta";
  ...
  const placeholderValor = distanciaOuPace && modo === "pace" ? "ex.: 5:30" : "ex.: 10";
        ...
        {distanciaOuPace && (
          <div className="flex gap-2 text-sm">
            <label className="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-[var(--radius-badge)] border border-track-fog/40 px-3 py-1.5 has-[:checked]:border-stadium-blue has-[:checked]:bg-stadium-blue/5">
              <input type="radio" name="modo" value="distancia" checked={modo === "distancia"} onChange={() => setModo("distancia")} />
              Por distância
            </label>
            <label className="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-[var(--radius-badge)] border border-track-fog/40 px-3 py-1.5 has-[:checked]:border-stadium-blue has-[:checked]:bg-stadium-blue/5">
              <input type="radio" name="modo" value="pace" checked={modo === "pace"} onChange={() => setModo("pace")} />
              Por pace
            </label>
          </div>
        )}
